...
class PDFController extends Controller
{
    ...
    public function generateCoorProfPdf($Prof_Name, $Prof_School){
      ...
      $profId = Prof_sched::where('profName', $Prof_Name)->where('profSchool', $schoolName)->value('profId');

      $employment = Prof_config::where('profId', $profId)->value('employment');
      
      $units = Prof_sched::where('profName', $Prof_Name)->where('profSchool', $schoolName)->sum('totalHours');

      $coorProfSchedules = Prof_sched::where('profName', $Prof_Name)->where('profSchool', $Prof_School)->orderBy('startTime', 'asc')->get();

      $pdf = Pdf::loadView('coor.pdf-prof-coor', ['coorProfSchedules' => $coorProfSchedules, 'schoolName' => $schoolName, 'schAddress' => $schoolAddress, 'schLogo' => $schLogo, 'schContact' => $schoolCntcts, 'employment' => $employment, 'units' => $units])->setPaper('a4', 'landscape');
      return $pdf->stream();
    ...
    public function generatePdfProfSched(){
      ...
      $schoolName = Prof_info::where('id', $profId)->value('profSchool');

      $schoolCity = School_info::where('schName', $schoolName)->value('city');
      $schoolBrgy = School_info::where('schName', $schoolName)->value('barangay');
      $schoolStrt = School_info::where('schName', $schoolName)->value('street');

      $schoolAddress = $schoolStrt . " " . $schoolBrgy . ", " . $schoolCity;
       
      $schLogo = School_info::where('schName', $schoolName)->value('schLogo');

      $schoolContactNum = School_info::where('schName', $schoolName)->value('contactNumber');
      $schoolEmail = School_info::where('schName', $schoolName)->value('email');
    ...
    //   $profId = Prof_sched::where('profName', $Prof_Name)->where('profSchool', $schoolName)->value('profId');
      ...
      $units = Prof_sched::where('profId', $profId)->where('profSchool', $schoolName)->sum('totalHours');

      $profSchedules = Prof_sched::where('profId', $profId)->orderBy('schedDay', 'asc')->get();

      $pdf = Pdf::loadView('prof.prof-pdf', ['profSchedules' => $profSchedules, 'schoolName' => $schoolName, 'schoolAddress' => $schoolAddress, 'schLogo' =>$schLogo, 
      'schoolCntcts' => $schoolCntcts, 'employment' => $employment, 'units' => $units])->setPaper('a4', 'landscape');
      return $pdf->stream();
